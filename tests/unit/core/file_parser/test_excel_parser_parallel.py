...
class TestExcelParserParallel:
    ...
    def test_parse_very_large_file_with_sheet_partitioning(self, tmp_path):
        ...
        with patch('os.path.getsize') as mock_getsize:
            ...
            # Instead of testing the actual partitioning logic, which is complex,
            # we'll test that the parser correctly calls the standard parse method
            # when partitioning is not possible
            with patch.object(parser, '_parse_large_file') as mock_parse_large_file:
                # Make the mock return the original DataFrame
                mock_parse_large_file.return_value = df.copy()

                # Mock the _can_partition_sheet method to return False
                # This will force the parser to use the standard method
                with patch.object(parser, '_can_partition_sheet', return_value=False):
                    # Parse the file
                    result = parser.parse()

                    # Check that the standard parse method was called
                    mock_parse_large_file.assert_called_once()
