...
class TestValueNormalizer:
    ...
    def test_normalize_prices(self):
        ...
        assert isinstance(result.iloc[1]['Trade Price'], (int, float))
        assert isinstance(result.iloc[2]['Trade Price'], (int, float))
        assert pd.isna(result.iloc[3]['Trade Price'])
        
        assert isinstance(result.iloc[0]['MSRP'], (int, float))
        assert isinstance(result.iloc[1]['MSRP'], (int, float))
        assert isinstance(result.iloc[2]['MSRP'], (int, float))
        assert pd.isna(result.iloc[3]['MSRP'])
        
        # Check specific values
        assert result.iloc[0]['Trade Price'] == 299.99
        assert result.iloc[2]['Trade Price'] == 499.0
    ...
    def test_normalize_ids(self):
        ...
        assert result.iloc[0]['UPC'] == '123456789012'
        assert result.iloc[1]['UPC'] == '123456789012'
        assert result.iloc[2]['UPC'] == '123456789012'
        assert pd.isna(result.iloc[3]['UPC'])
