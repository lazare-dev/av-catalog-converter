    ...
    def _standardize_existing_categories(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in result_df.iterrows():
            ...
            if pd.notna(current_cat) and current_cat in category_mapping:
                # Set standard category if available
                if "standard_category" in category_mapping[current_cat]:
                    result_df.at[idx, "Category"] = category_mapping[current_cat]["standard_category"]

                # Set standard group if available
                if "standard_group" in category_mapping[current_cat]:
                ...
                # Set standard group if available
                if "standard_group" in category_mapping[current_group]:
                    result_df.at[idx, "Category Group"] = category_mapping[current_group]["standard_group"]

                # Only set category if it's currently empty and standard_category is available
    ...
    def _derive_category(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if present_fields:
            ...
            for idx, row in result_df.iterrows():
                ...
                # Fix: Ensure group is a scalar value, not a Series
                if isinstance(group, pd.Series):
                    group = group.iloc[0] if not group.empty else None

                # Check if group is valid
                if pd.isna(group) or not isinstance(group, str) or group not in group_mapping:
                    continue

                # Ensure we have a valid standard_group
                if "standard_group" not in group_mapping[group]:
                    continue
    ...
    def _infer_categories(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for batch_start in range(0, len(result_df), batch_size):
            ...
            for idx, categories in category_results.items():
                ...
                # Fix: Ensure we're getting valid string values for categories
                category = categories.get("category")
                category_group = categories.get("category_group")

                if category is not None and not pd.isna(category):
                    result_df.at[idx, "Category"] = str(category)

                if category_group is not None and not pd.isna(category_group):
                    result_df.at[idx, "Category Group"] = str(category_group)
