    ...
    def map_to_standard_taxonomy(self, categories: List[str] = None,
        ...
        if categories is not None and len(categories) > 0:
            for category in categories:
                ...
                if category is None or (isinstance(category, (float, int)) and pd.isna(category)) or (isinstance(category, str) and not category.strip()):
                    continue

                # Convert to string if not already
                category_str = str(category).strip()
                std_category = self._find_matching_category(category_str)

                mapping[category_str] = {
                ...
                if infer_groups or std_category:
                    ...
                    if group:
                        mapping[category_str]["standard_group"] = group

        # Map groups
        if category_groups is not None and len(category_groups) > 0:
            for group in category_groups:
                # Handle NaN values properly
                if group is None or (isinstance(group, (float, int)) and pd.isna(group)) or (isinstance(group, str) and not group.strip()):
                    continue

                # Convert to string if not already
                group_str = str(group).strip()
                std_group = self._find_matching_group(group_str)

                if group_str not in mapping:
                    mapping[group_str] = {
                        "original": group_str,
                        "standard_group": std_group
                    }
                else:
                    mapping[group_str]["standard_group"] = std_group

                # Add a default category if requested
                if infer_categories and std_group:
                    std_categories = self.standard_taxonomy.get(std_group, [])
                    if len(std_categories) > 0:
                        mapping[group_str]["standard_category"] = std_categories[0]
